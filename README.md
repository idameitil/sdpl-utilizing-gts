...
The architectures were manually written from the alphafold structures in pymol into `data/compare-architectures/architecture-tables/[acc].csv`.

The architecture and conserved residue is generated by running: `python src/compare-architectures/make-architecture-data.py`. This creates the files `architecture.js`and `conserved_[threshold].js`.
...
## WadA
The sequence of the WadA from "Lipopolysaccharide biosynthesis and traffic in the envelope of the pathogen Brucella abortus" was written into `data/wada/wadA.fa`. 
...
`blastfilter -js data/wada/blast.out > data/wada/blast.js`

`python src/wada/make-hits-file.py`

`data/wada/hits-e-40.txt`
