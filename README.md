...
(DON'T RUN THIS - the file has been changed manually) The architecture file is generated by running: `python src/compare-architectures/make-architecture-data.py`. This creates the files `architecture.js`.

The residues that we want to show on the figure, are written in `data/compare-architectures/conserved_manual_offsets.js`. This is the data file read by the html script.
