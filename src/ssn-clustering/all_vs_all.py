...
if os.path.exists('unique_hits_filtered.fasta'):
	...
	for line in infile:
		...
		if acc in hit2evalue_series.index and len(seq) > 320 and len(seq) < 500 and seq.startswith("M"):
			outfile.write(f'>{acc}\n{seq}\n')
	...
	def write_fasta(threshold):
		...
		with open(filename, 'w') as fp:
			...
			for acc in annotated:
				if acc not in accessions_include and f"{acc}.1" not in accessions_include: # only add those that are not already there
					seq = annotated[acc]
					fp.write(f'>{acc}\n{seq}\n')
		fp.close()
